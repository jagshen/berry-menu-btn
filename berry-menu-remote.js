...
(function () {
  ...
  // 兼容：如果桥接未就绪，自行实现基本功??  var _isBerry = config.isBerry || !!(typeof _page.BerryBrowser !== 'undefined' || _page._berry_homepage);
  ...
  function setupDisplayMethodFloat(shadow, isFlash) {
    ...
    // 移除旧热??    if (hotZone) hotZone.remove();
    if (savedMethod === 'always') {
  ...
  function createHotZone(targetBtn, method, isShadow, shadow, isFlash) {
    ...
    if (method === 'longpress') {
      ...
      // 常驻模式：点击菜单按钮区域打开菜单，其他区域穿??      hot.addEventListener('click', function(e) {
        // 检查是否在菜单按钮范围内（按钮位置：top:30px/45px, left:16px, 大小??8x28??        var btnTop = (isShadow && !isHomePage()) ? '30px' : '45px';
        var btnLeft = 16;
        var btnSize = 28;
    ...
    // 只在用户手动切换时闪??    if (isFlash) {
      // 热区闪烁效果??次闪烁，每次300ms??      var flashCount = 0;
      var maxFlashes = 3;
  ...
  /* ════════════════════════════════════════
     主页场景：通过 BerryHomeMenu API 追加菜单??     ════════════════════════════════════════ */
...
function bindSwitchMethodEvents(sectionEl, menuApi) {
  ...
  function doEnhance(menuApi) {
    if (_DEBUG) console.log('[berry-remote] 开始增强原生菜??);
    ...
    if (modeLabel) {
      ...
      // 把原??#menuTip 移入 .mode-label，实现同行显??      if (menuTip) {
        menuTip.style.margin = '0';
        menuTip.style.padding = '0 3px';
        modeLabel.appendChild(menuTip);
      }

      // 【关键】覆盖原??showTip，防止原生逻辑??#menuTip 移回原位??      var _origShowTip = menuApi.showTip;
      menuApi.showTip = function (msg) {
        // 先调用原??showTip（它可能会移??DOM??        if (typeof _origShowTip === 'function') {
    ...
    // 注入 CSS 覆盖 .menu-tip 行内样式（不??HTML??    if (!_doc.getElementById('_berryMenuTipCSS')) {
      ...
      tipStyle.textContent = [
        ...
        /* 对齐原生 .menu-tip 样式：大方块 + ??opacity 淡入 */
    ...
    // 为默认样式添加点击提??    var defaultItem = _doc.getElementById('styleDefault');
    if (defaultItem) {
    ...
    var switchSectionHTML =
      ...
      '<div class="switch-method-item' + (savedMethod === 'longpress' ? ' active' : '') + '" data-method="longpress"><span>??</span><span>长按</span></div>' +
  ...
  function initFloatingMenu() {
    ...
    if (_DEBUG) console.log('[berry-remote] 悬浮按钮已创??);

    // 初始化显示方??    setupDisplayMethodFloat(shadow);
    ...
    // 添加自定义链接按钮事??    var applyBtn = shadow.querySelector('#floatApplyBtn');
  ...
  function getMenuHTML(savedMethod, savedStyle, savedCustomUrl) {
    ...
    return (
      ...
      '<div class="f-switch-method-item' + (savedMethod === 'longpress' ? ' active' : '') + '" data-fm="longpress" onclick="__berryHandleSwitchMethod(\'longpress\')"><span>??</span><span>长按</span></div>' +
